...
async function discoverLog(url,artifact) {
    if (! artifact) {
        let log = await discoverLinkHeaderLog(url,artifact);
...
async function discoverLinkHeaderLog(url,artifact) {
    try {
        ...
        if (linkHeader) {
            ...
            for (let i = 0 ; i < EVENT_LOG_RELATION.length ; i++) {
                ...
                const header = parsedLinkHeaders.get('rel',rel);
        ...
        if (htmlLinks) {
            const log = htmlLinks.find( (link) => {
                if (link['rel'] === 'eventlog' && link['href']) {
                    return true;
                }
            });
            return log['href'];
...
async function discoverLinkTemplateLog(url,artifact) {
    try {
        logger.info(`discoverLinkTemplateLog(${url},${artifact})`);
        ...
        logger.info(`trying to find an link-template in the RDF of ${url}`);
...
async function discoverCatalogLog(url,artifact) {
    try {
        logger.info(`discoverCatalogLog(${url},${artifact})`);
        ...
        logger.info(`trying to find an eventlog_catalog in the RDF of ${url}`);
